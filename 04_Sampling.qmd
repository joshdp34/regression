...
::: {.callout-note collapse="false" title="Big picture: from assumptions to inference"}

The pieces in this chapter fit together like this:

```{mermaid}
flowchart LR
  A["Model assumptions<br/>mean zero errors<br/>constant variance<br/>normality<br/>independence"] --> B["Properties of least squares<br/>unbiased estimators<br/>variance formulas<br/>BLUE"]
  B --> C["Sampling distribution<br/>how b1 varies<br/>from sample to sample"]
  C --> D["Studentized score<br/>replace unknown sigma<br/>with estimated s"]
  D --> E["Slope inference<br/>t test for beta1<br/>confidence interval"]
```

The important story is not just that formulas exist. The assumptions give us a probability model for the data; that probability model gives us a sampling distribution for $b_1$; and that sampling distribution makes hypothesis tests and confidence intervals possible.
